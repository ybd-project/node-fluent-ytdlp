...
let automaticDownloadMessage = 'yt-dlp・ffmpeg・ffprobeを自動でダウンロードしますか？ ';
if (Intl.NumberFormat().resolvedOptions().locale !== 'ja-JP') {
    automaticDownloadMessage = 'Automatically download yt-dlp, ffmpeg, and ffprobe ';
}
inquirer_1.default
    .prompt([
    {
        ...
        default: 'yes',
        message: automaticDownloadMessage,
    },
])
    .then(({ automaticDownload }) => {
    if (automaticDownload === 'yes') {
        const urls = (() => {
        ...
        console.log('yt-dlp・ffmpeg・ffprobeは自動でダウンロードされません。');
    }
})
    .catch((err) => {
    console.log('エラーが発生しました。' + err);
    console.log('yt-dlp・ffmpeg・ffprobeは自動でダウンロードされます。');
});
