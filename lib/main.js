...
const { binaryPath, os } = JSON.parse(node_fs_1.default.readFileSync(node_path_1.default.join(__dirname + '/../bin/info.json'), 'utf8')), noParamText = 'Option with no parameters.';
//yt-dlpのオプションを生成する
...
(0, update_notifier_1.default)({ pkg: { name: 'node-fluent-ytdlp', version: '1.0.0' }, updateCheckInterval: 1000 }).notify({
    message: '更新情報: ' +
        chalk_1.default.yellow('{currentVersion}') +
        chalk_1.default.reset(' → ') +
        chalk_1.default.green('{latestVersion}') +
        '\n更新方法: ' +
        chalk_1.default.cyan('{updateCommand}'),
